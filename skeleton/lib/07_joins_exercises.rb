  ...
  execute(<<-SQL)
    ...
    JOIN (
      ...
      FROM
        ...
        ON c.actor_id = a.id
        JOIN castings ca
        ON c.movie_id = ca.movie_id
      WHERE
        a.name = 'Art Garfunkel'
      ) as art_cocastings
      ON a.id = art_cocastings.actor_id
      WHERE art_cocastings.id != a.id
